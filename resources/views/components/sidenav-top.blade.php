    ...
    <div class="container py-2">
        <nav aria-label="breadcrumb">
            <div class="d-flex align-items-center">
                <span class="px-3 font-weight-bold text-sig text-white me-4">SIG</span>
            </div>
        </nav>
                ...
                <div class="d-flex align-items-center w-sm-45" style="margin-left: 345px; margin-top: 20px; height: 30px;">
                    <div class="input-group border-dark">
                        ...
                        <input type="text" class="form-control border-dark bg-light text-white" name="search" placeholder="Search"
                                onfocus="focused(this)" onfocusout="defocused(this)" style="height: 37px;">
                    </div>
                </div>
            </form>
            <li class="nav-item px-3 d-flex align-items-center ms-auto mt-3" style="margin-right: -15px;">
                <form action="{{ route('logout_karyawan') }}" method="POST" class="nav-link text-white p-0">
                    @csrf
        ...
        @elseif (auth()->user()->level=="Admin")
            ...
            <li class="nav-item px-3 d-flex align-items-center ms-auto mt-3" style="margin-right: -15px;">
                <form action="{{ route('logout_admin') }}" method="POST" class="nav-link text-white p-0">
                    @csrf
                    <button type="submit" class="btn btn-link btn-lg">
                        <i class="fas fa-power-off fixed-plugin-button-nav cursor-pointer text-white"></i>
                    </button>
